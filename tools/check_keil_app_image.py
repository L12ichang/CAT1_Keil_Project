...
def parse_c_define_int(source: str, name: str) -> int | None:
    match = re.search(rf"#define\s+{re.escape(name)}\s+(?:\(u32\))?\s*(0x[0-9A-Fa-f]+|\d+)", source)
...
def check_source_contracts(report: CheckReport) -> None:
    ...
    for name, expected in expected_values.items():
        actual = parse_c_define_int(sys_data_text, name)
...
def check_keil_outputs(project_path: Path, report: CheckReport, require_fresh: bool) -> None:
    ...
    if require_fresh:
        ...
        # cat1.sct is a checked-in/static scatter input in this Keil project;
        # unlike map/bin/hex it is not regenerated by a clean rebuild.
