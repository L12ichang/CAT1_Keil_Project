...
> 基于《程序架构编写规范.docx》及 2021.12.11 原始参考代码提炼，适用于 STM32/PT32 系列嵌入式固件项目。
...
- ✅ 系统层调用硬件层
- ✅ 应用层调用系统层
- ❌ 应用层**禁止**直接调用硬件层（功能特别简单的除外，如 led_on/led_off）
- ❌ 硬件层**禁止**调用系统层和应用层
...
| 作用域 | 命名规范 | 示例 |
|--------|----------|------|
| 全局变量 | 模块名_描述 | `sys_data`, `breathing_light_state` |
...
- [ ] 模块文件命名符合规范（hw_/sys_/无前缀）
- [ ] 函数命名使用模块前缀
- [ ] include 守卫使用 `__XXX_H` 双下划线格式
- [ ] while(1) 第一行是 watchdog_feed_dog()
- [ ] 无死等循环（sys_tick_delay 除外）
- [ ] ISR 中无业务逻辑调用
