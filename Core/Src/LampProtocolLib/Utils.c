#include "Utils.h"
#include "stdio.h"
#include "string.h"
#include "Portable.h"



/**
*@brief   转换Hex字符串为hex数组
*@param	  pInHexString：输入hex字符串
*@param	  nInLen：输入hex字符串长度
*@param	  pOut：输出hex数组
*@param	  nOutLen：输出hex数组长度
*@return  转换成功：返回1，转换失败：返回0
*/
uint8 hexStrToByte(const char *pInHexString,  uint16 nInLen, uint8 *pOut, uint16 *nOutLen) 
{
     int8 Temp[2];
     uint16 nIndex = 0;
     uint16 i = 0, j = 0;
     uint8 nData = 0;
     uint8 nOut = 0;
    if (pInHexString == NULL || pOut == NULL || (nInLen % 2)) 
    {
        return 0;
    }

    for (i = 0; i < nInLen; i += 2) 
    {
        Temp[0] = pInHexString[i];
        Temp[1] = pInHexString[i + 1];

        for (j = 0; j < 2; ++j) 
        {
            switch (Temp[j]) 
            {
                case '0':
                case '1':
                case '2':
                case '3':
                case '4':
                case '5':
                case '6':
                case '7':
                case '8':
                case '9':
                    nData = Temp[j] - '0';
                    nOut += nData << ((1 - j) * 4);
                    break;
                case 'A':
                case 'B':
                case 'C':
                case 'D':
                case 'E':
                case 'F':
                    nData = Temp[j] - 'A' + 10;
                    nOut += nData << ((1 - j) * 4);
                    break;
                case 'a':
                case 'b':
                case 'c':
                case 'd':
                case 'e':
                case 'f':
                    nData = Temp[j] - 'a' + 10;
                    nOut += nData << ((1 - j) * 4);
                    break;
                default:
                    return 0;
            }
        }
        pOut[nIndex++] = nOut;
        nOut = 0;
    }
    *nOutLen = nIndex;
    return 1;
}

/**
*@brief   转换hex数组为Hex字符串
*@param	  source：输入的hex数组
*@param	  dest：输出的hex字符串
*@param	  sourceLen：输入的hex数组长度
*@return  无
*/
void byteToHexStr(const unsigned char *source, char *dest, int sourceLen) 
{
    uint16 i;
    uint8 highByte, lowByte;

    for (i = 0; i < sourceLen; i++) 
    {
        highByte = source[i] >> 4;
        lowByte = source[i] & 0x0f;

        highByte += 0x30;

        if (highByte > 0x39)
            dest[i * 2] = highByte + 0x07;
        else
            dest[i * 2] = highByte;

        lowByte += 0x30;
        if (lowByte > 0x39)
            dest[i * 2 + 1] = lowByte + 0x07;
        else
            dest[i * 2 + 1] = lowByte;
    }
    return;
}


static unsigned char const  _TTS_Crc16_TableH[] =
{
        
};
/* CRC 低位字节值表*/
static unsigned char const _TTS_Crc16_TableL[] =
{
        
};

unsigned char TempCrcH = 0xFF;
unsigned char TempCrcL = 0xFF;
unsigned char CrcIndex;
/**
*@brief   计算crc16-modbus校验和
*@param	  DataSource：待计算的数组
*@param	  DataLen：数组长度
*@return  crc计算结果
*/
unsigned short getCRC16(unsigned char *DataSource, unsigned short DataLen)
{
    unsigned short i, ReValue;
	TempCrcH = 0xFF;
	TempCrcL = 0xFF;
    for (i = 0; i < DataLen; i++) 
    {
        CrcIndex = TempCrcL ^ (*DataSource++);
        TempCrcL = TempCrcH ^ _TTS_Crc16_TableH[CrcIndex];
        TempCrcH = _TTS_Crc16_TableL[CrcIndex];
    }
    ReValue = TempCrcH;
    ReValue <<= 8;
    ReValue |= TempCrcL;
    return ReValue;
}

/**
*@brief   初始化crc16-modbus算法，该方法用于计算长数组的校验码
*@return  无
*/
void initCalcCrc16(void)
{
	TempCrcH = 0xFF;
	TempCrcL = 0xFF;
}

/**
*@brief   计算crc16-modbus校验码，该方法用于计算长数组的校验码
*@param	  DataSource：待计算的数组
*@param	  DataLen：数组长度
*@return  无
*/
void calcCrc16(unsigned char *DataSource, unsigned short DataLen)
{
	unsigned short i;
    for (i = 0; i < DataLen; i++)
    {
        CrcIndex = TempCrcL ^ (*DataSource++);
        TempCrcL = TempCrcH ^ _TTS_Crc16_TableH[CrcIndex];
        TempCrcH = _TTS_Crc16_TableL[CrcIndex];
    }
}

/**
*@brief   返回crc16-modbus校验码，该方法用于计算长数组的校验码
*@return  crc计算结果
*/
unsigned short getCrc16Result(void)
{
   return TempCrcH << 8 | TempCrcL;
}
