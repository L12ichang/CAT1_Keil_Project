...
/*
 * Program a range in an already-erased page.  Persistent A/B records use
 * this entry so the commit word can be programmed after the verified record
 * body without a second page erase.
 */
boolean_en hw_flash_program_bytes_no_erase_checked(uint32_t flash_addr,
                                                   const u8 *buffer,
                                                   uint32_t length)
{
    u32 offset;
    ...
    HAL_FLASH_Lock();
    if (result == BOOL_TRUE &&
        user_flash_check(flash_addr, (u8 *)buffer, (u16)length) != BOOL_TRUE)
    {
        result = BOOL_FALSE;
    }
    return result;
...
boolean_en hw_flash_erase_page_checked(uint32_t page_address)
{
    FLASH_EraseInitTypeDef erase_init;
    u32 page_error = 0U;
    HAL_StatusTypeDef status;

    if ((page_address & (FLASH_PAGE_SIZE - 1U)) != 0U ||
    ...
    status = HAL_FLASHEx_Erase(&erase_init, &page_error);
    HAL_FLASH_Lock();
    return (status == HAL_OK) ? BOOL_TRUE : BOOL_FALSE;
}
