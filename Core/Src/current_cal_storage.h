...
/* During an interrupted guarded repair, identifies the only page that may be
 * rewritten without making an ambiguous calibration record selectable. */
...
/* Calibration-only primitive for the peer writer.  It never invokes the
 * registered callback and therefore cannot recurse. */
